$(function () {
        ...
        onClick: function () {
            DevExpress.ui.dialog.confirm("Are you sure you want to reset the form?", "Reset Confirmation").done(function (result) {
                if (result) {
                    ...
                    dateOfBirth.reset();
                    gender.reset();  
                    maritalStatus.reset(); // ✅ Fixed: Reset marital status
                    address.reset(); // ✅ Fixed: Reset address
                    contactNo.reset(); // ✅ Fixed: Reset contact number
                    tenthResult.reset(); // ✅ Fixed: Reset 10th result
                    twelthResult.reset(); // ✅ Fixed: Reset 12th result
                    department.reset(); // ✅ Fixed: Reset department
                    course.option("items", []).reset(); // ✅ Fixed: Reset course selection
                    profile.reset(); // ✅ Fixed: Reset file uploader
                    validate.reset(); // ✅ Fixed: Reset checkbox
    ...
    ////  reset not work properly
    // gender is not store into session storage 
    ...
    // profilePhoto.registerKeyHandler("enter", function () { validate.focus(); });
